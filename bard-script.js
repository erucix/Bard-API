...
var cookie = "REPLACE_WITH YOUR __SECURE-1PSID COOOKIE";  //       <--------REPLACE HERE
    ...
    return new Promise(function (resolve) {
        APICall({
            ...
            if (data.status == "success") {
                ...
                SNlM0e.message = "REPLACE-WITH-YOURSNLM0E-TOKEN"; //   <----------REPLACE HERE
    ...
    return new Promise(function (resolve) {
        if (options.method == "POST") {
            ...
            https.get(options.uri, optionsForRequest, (responseFromServer) => {
                ...
                let setCookieSentByServer = responseFromServer.headers["set-cookie"];

                setCookieSentByServer.forEach(element => {
                    fullCookie.push(element.slice(0, element.indexOf(";") + 1));
                })

                cookie += fullCookie.join(";");

                addToSavedPreference("cookie", cookie);
...
function createInterface() {
    cookie = (getSavedPreference().message.cookie || cookie);

    getBardApiTokens()
        .then(data => {
            rl.question("💀 \x1b[1;32mYou\x1b[0m\x1b[1;33m:\x1b[0m \x1b[1;35m", (message) => {
                if (message.startsWith("$") || message.trim() == "") {
                    ...
                    if (message == "clear") {
                        ...
                        let old_c_id = getSavedPreference().message.c_id;

                        getChatHistory(old_c_id, { "snlm0e": data.message.snlm0e })
                            .then(data => {
                                data = data.message;
                                data.forEach((element) => {
                                    console.log("💀 \x1b[1;32mYou\x1b[0m\x1b[1;33m:\x1b[0m \x1b[1;35m" + element.you);
                                    console.log("🙂 \x1b[1;34mBot\x1b[0m\x1b[1;33m:\x1b[0m \x1b[1;36m" + element.bard)
                                })
                                createInterface();
                            })
                    } else if (message == "history") {

                        getBardChatTitleHistory({
                            "snlm0e": data.message.snlm0e
                        }).then(data => {
                            if (data.status == "fail") createInterface();

                            data = data.message;
                            data.length > 0 && data.forEach((element, index) => {
                                console.log(`\x1b[0m\x1b[1;33m[\x1b[0m\x1b[1;34m${index + 1}\x1b[0m\x1b[1;33m]\x1b[0m ${element.title}`)
                            });
                            if (data.length > 0) {
                                rl.question("> ", (message) => {
                                    if (message > 0 && message <= data.length) {
                                        let new_c_id = data[message - 1].id;
                                        addToSavedPreference("c_id", new_c_id);
                                        addToSavedPreference("r_id", "");
                                        addToSavedPreference("rc_id", "");
                                        console.log("Connected to chat id: " + new_c_id);
                                    }
                                    createInterface();
                                })
                            } else {
                    ...
                    prompt({
                        ...
                        console.log(`🙂 \x1b[1;34mBot\x1b[0m\x1b[1;33m:\x1b[0m \x1b[1;36m${data.message.answer}`)
                        createInterface(data.message);
                    })
                }
            });
        });
